...
if city == "nyc":
    rich_count = 15  # Number of rich nodes to consider for the rich club density
elif city == "tky":
    rich_count = 15
...
if not sliding_window:
    if not os.path.exists(f"properties_{city}__k_{k_days}"):
        os.makedirs(f"properties_{city}__k_{k_days}")
else:
    if not os.path.exists(f"SLIDING_properties_{city}__k_{k_days}"):
        os.makedirs(f"SLIDING_properties_{city}__k_{k_days}")
...
for seed_dir in version_dir.iterdir():
    ...
    if not sliding_window:
        colocation_network_simulated_dir = seed_dir / 'colocationnetworks' / f"city_{city}__train_{t_window}__step_{k_days}__max_{max_simulation_days}__topK_{topK}__alg_{algorithm}"
    else:
        colocation_network_simulated_dir = seed_dir / 'colocationnetworks' / f"SLIDING_city_{city}__train_{t_window}__step_{k_days}__max_{max_simulation_days}__topK_{topK}__alg_{algorithm}"
...
if not sliding_window:
    dirname = f"properties_{city}__k_{k_days}"
else:
    dirname = f"SLIDING_properties_{city}__k_{k_days}"

with open(f"{dirname}/nodes_coloc.csv", "a+") as f:
    f.seek(0)
...
for i, ((x, y), AggG) in enumerate(zip(all_distributions, networks)):
    ...
    if np.sum(mask) >= 3:  # Need at least 3 points for meaningful fit
        ...
        with open(f"{dirname}/gamma.csv", "a+") as f:
...
save_rich_club_density(agg_networks_dict, rich_count=rich_count, city=city, algorithm=algorithm,
                       output_file=f"{dirname}/rich_club_density.csv")
...
random.seed(42)
new_columns = ['uid', 'lat', 'lon', 'venueID', 'venue_descr', 'time', 'city_name', 'First_Category', 'Second_Category',
               'step', 'epoch', 'city', 'simulation_state']
...
for p in ps:
    if not sliding_window:
        ...
        experiment_dir = f"../data/processed/experiments/{version}/{seed}/SLIDING_city_{city}__train_{t_window}__step_{k_days}__max_{max_simulation_days}__topK_{topK}__alg_{algorithm}__recProb_{p}"
...
with open(f"{dirname}/gini_visits.csv", "a+") as f:
    f.seek(0)  # Move to beginning of file
    file_content = f.read()
...
with open(f"{dirname}/gini_trajectories.csv", "a+") as f:
    f.seek(0)  # Move to beginning of file
    file_content = f.read()
    if f"{seed}, {algorithm}" not in file_content:
